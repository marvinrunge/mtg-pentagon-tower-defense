...
func _run_spell_effect(spell_id: String, charge_pct: float, held: bool = false) -> void:
	...
	match spell_id:
		...
		"blue_3": cast_blue_suction()
		"blue_4": cast_blue_wall_of_frost()
		"blue_5": cast_blue_displace()
		# --- BLACK ---
...
func cast_red_fireball(charge_pct: float) -> void:
	...
	var spawn_pos: Vector3 = _charge_muzzle if _charge_muzzle != Vector3.ZERO else camera.global_position - camera.global_basis.z * 1.5
	...
	ProjectilePool.fire(spawn_pos, dir, 4, false, mult, -1.0, radius, self)
	...
	_play_sound(&"spell_cast", global_position)
...
func cast_green_titanic_leap() -> void:
	...
	_play_sound(&"blade_heavy_swing", global_position)
...
func _slam_ground() -> void:
	_leap_timer = 0.0
	_play_sound(&"heavy_landing", global_position)
	_shake(
...
func cast_red_rain_ember() -> void:
	...
	_place_networked({
		...
		"radius": GameSettings.spell_red_rain_ember_radius * _rank_area(),
		"dps": GameSettings.spell_red_rain_ember_dps * get_spell_damage_multiplier() * _rank_damage(),
		"duration": GameSettings.spell_red_rain_ember_duration * _rank_duration(),
	})
...
func cast_white_exalted_strike() -> void:
	...
	_play_sound(&"spell_exalted_strike", global_position)
...
func cast_white_circle_of_protection() -> void:
	...
	if allies.is_empty():
		return
	var each: float = GameSettings.player_max_hp * GameSettings.spell_white_circle_shield_hp_mult * _rank_damage()
	each *= 1.0 + GameSettings.spell_white_circle_ally_bonus * float(allies.size() - 1)
	var duration: float = GameSettings.spell_white_circle_duration * _rank_duration()
	for ally: Node3D in allies:
		if ally.has_method("grant_protection_shield"):
			...
			_credit_heal(ally, ally.heal(each))
		_spawn_ring(ally.global_position, FX_WHITE, 1.6)
	_play_sound(&"spell_circle_protection", global_position)
...
func cast_white_reprisal_ward() -> void:
	...
	_play_sound(&"spell_reprisal_ward", global_position)
...
func cast_white_wrath_of_god() -> void:
	...
	_play_sound(&"spell_wrath_of_god", global_position)
	_shake(0.6, 0.4)
	...
	_spawn_impact(global_position, FX_WHITE, radius)
...
func cast_white_rally_the_fallen() -> void:
	var heal_amount: float = GameSettings.player_max_hp * GameSettings.spell_white_rally_heal_hp_mult * _rank_damage()
	...
	for ally: Node3D in _allies_in_radius(GameSettings.spell_white_rally_radius * _rank_area()):
		...
		if ally.has_method("heal"):
			_credit_heal(ally, ally.heal(heal_amount))
	...
	_play_sound(&"spell_rally_fallen", global_position)
...
func cast_blue_unsummon() -> void:
	...
	facing.y = 0.0
	_spawn_gust(
		global_position + Vector3(0.0, 1.0, 0.0), facing,
		GameSettings.spell_blue_unsummon_range, FX_BLUE
	)
	_play_sound(&"spell_unsummon", global_position)
	if pushed > 0:
		_play_sound(&"blunt_hit", global_position)
...
func cast_blue_frostwave() -> void:
	...
	_play_sound(&"spell_frostwave", global_position)
	_shake(0.35, 0.3)
...
func cast_blue_suction() -> void:
	...
	_play_sound(&"spell_suction", center)


## blue_4. A solid ice barricade laid perpendicular to the aim direction, across the lane.
func cast_blue_wall_of_frost() -> void:
	var center: Vector3 = _aim_point(12.0)
	var forward: Vector3 = center - global_position
	forward.y = 0.0
	...
	forward = forward.normalized()
	_place_networked({
		"kind": "wall_of_frost", "position": _ground_snap(center),
		"length": GameSettings.spell_blue_wall_of_frost_length * _rank_area(),
		"duration": GameSettings.spell_blue_wall_of_frost_duration * _rank_duration(),
		"damage": GameSettings.spell_blue_wall_of_frost_unsummon_bonus_damage * get_spell_damage_multiplier() * _rank_damage(),
	...
	_play_sound(&"spell_frost_globe", center)


## blue_5. Displace: a short blue blink to the aimed ground point.
func cast_blue_displace() -> void:
	var from: Vector3 = global_position
	var max_distance: float = GameSettings.spell_blue_displace_distance * _rank_area()
	var target: Vector3 = _ground_snap(_aim_point(max_distance))
	var offset: Vector3 = target - from
	offset.y = 0.0
	if offset.length() > max_distance:
		target = from + offset.normalized() * max_distance
		target = _ground_snap(target)
	_spawn_ring(from, Color(0.42, 0.72, 1.0), 1.7)
	global_position = target
	velocity = Vector3.ZERO
	_spawn_ring(global_position, Color(0.68, 0.9, 1.0), 2.0)
	_play_sound(&"spell_cast", global_position)
...
func cast_black_doom_blade() -> void:
	...
	_play_sound(&"spell_doom_blade", global_position)
...
func cast_black_fear() -> void:
	...
	var duration: float = GameSettings.spell_black_fear_duration * _rank_duration()
	...
	var vulnerability: float = GameSettings.rank_fraction(
		GameSettings.spell_black_fear_vulnerability, GameSettings.spell_black_fear_vulnerability_max, _casting_rank
	)
	for enemy: Node3D in _enemies_in_radius(global_position, radius):
		if enemy.has_method("apply_fear"):
			enemy.apply_fear(duration, global_position)
	...
	_play_sound(&"spell_fear", global_position)
...
func cast_black_kill() -> void:
	...
	_play_sound(&"spell_kill", target.global_position)
	_shake(0.5, 0.3, target.global_position)
...
func cast_black_wall_of_souls() -> void:
	...
	var info: Dictionary = {
		...
		"length": GameSettings.spell_black_wall_length * _rank_area(),
		"duration": GameSettings.spell_black_wall_duration,
		"mark_duration": GameSettings.spell_black_wall_mark_duration * _rank_duration(),
	...
	_place_networked(info)
	_play_sound(&"spell_wall_of_souls", center)
...
func cast_black_zombify() -> void:
	...
	for corpse: EnemyBase in corpses:
		...
		_place_networked({
			"kind": "undead",
			"position": _ground_snap(where) + Vector3(0.0, 0.5, 0.0),
			"hp": GameSettings.spell_black_zombify_hp * _rank_damage(),
			"duration": GameSettings.spell_black_zombify_duration * _rank_duration(),
			"damage": GameSettings.spell_black_zombify_damage * get_spell_damage_multiplier() * _rank_damage(),
		...
		raised += 1
	_play_sound(&"spell_zombify", global_position)
...
func cast_red_fire_dash() -> void:
	...
	_play_sound(&"spell_fire_dash", global_position)
...
func _drop_trail_segment() -> void:
	...
	if _dash_trail_radius <= 0.0:
		return
	_place_networked({
		"kind": "dot_zone", "type": "fire_patch", "position": global_position,
		"radius": _dash_trail_radius, "dps": _dash_trail_dps, "duration": _dash_trail_duration,
	})
...
func cast_red_fire_cone(held: bool) -> void:
	if _channel_id != "" or spell_cooldown_timers.get("red_4", 0.0) > 0.0:
		return
	_channel_id = "red_4"
	_channel_timer = GameSettings.spell_red_fire_cone_max_duration
	_channel_held = held
	_channel_release_sent = false
	_channel_slot = active_spell_index
...
func cast_red_lightning_bolt() -> void:
	...
	var elite_mult: float = GameSettings.rank_fraction(
		GameSettings.spell_red_bolt_elite_mult, GameSettings.spell_red_bolt_elite_mult_max, _casting_rank
	)
	...
	timer.timeout.connect(func() -> void:
		...
		for enemy: Node3D in _enemies_in_radius(target, bolt_radius):
			_deal_damage(enemy, damage * _bolt_target_multiplier(enemy, elite_mult), false)
		...
		_play_sound(&"spell_lightning_bolt", target)
		_shake(0.55, 0.35, target))
...
func cast_green_giant_growth() -> void:
	...
	_giant_bonus_hp = GameSettings.player_max_hp * GameSettings.spell_green_giant_bonus_hp_mult * _rank_damage()
	...
	_play_sound(&"spell_giant_growth", global_position)
...
func cast_green_fog() -> void:
	...
	_place_networked({
		...
		"radius": GameSettings.spell_green_fog_radius * _rank_area(),
		"dps": 0.0,
		"duration": GameSettings.spell_green_fog_duration * _rank_duration(),
	})
...
func cast_green_roar() -> void:
	...
	_spawn_impact(global_position, FX_GREEN, radius)
	_play_sound(&"spell_roar", global_position)
	_shake(0.3, 0.3)
	if taunted > 0:
...
func cast_green_ironbark() -> void:
	...
	_play_sound(&"spell_ironbark", global_position)
